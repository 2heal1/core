...
rstest.mock('../core/index', () => ({
  validateOptions: rstest.fn(),
  generateTypes: rstest.fn().mockResolvedValue(undefined),
  generateTypesInChildProcess: rstest.fn().mockResolvedValue(undefined),
  retrieveTypesAssetsInfo: rstest.fn().mockReturnValue({
...
const { mockIsDev } = { mockIsDev: rstest.fn() };
rstest.mock('./utils', () => ({
  isDev: mockIsDev,
  getCompilerOutputDir: rstest.fn().mockReturnValue('dist'),
}));
...
function createMockCompiler() {
  ...
  const compilation = {
    hooks: {
      processAssets: {
        tapPromise: rstest.fn((_opts: unknown, fn: () => Promise<void>) => {
  ...
  const compiler = {
    ...
    hooks: {
      thisCompilation: {
        tap: rstest.fn((_name: string, fn: (c: typeof compilation) => void) => {
          fn(compilation);
        }),
      },
    },
...
describe('afterGenerate callback', () => {
  ...
  it('should call afterGenerate after type generation in prod', async () => {
    ...
    const afterGenerate = rstest.fn().mockResolvedValue(undefined);
  ...
  it('should call afterGenerate after type generation in dev', async () => {
    ...
    await rstest.waitFor(() => expect(afterGenerate).toHaveBeenCalledOnce());
  });
  ...
  it('should await async afterGenerate before continuing', async () => {
    mockIsDev.mockReturnValue(false);
    const order: string[] = [];
    const afterGenerate = rstest.fn(async () => {
      order.push('callback');
    });
    const { compiler, triggerProcessAssets } = createMockCompiler();
