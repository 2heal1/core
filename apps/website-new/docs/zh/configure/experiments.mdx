...
设置 `true` 后 会 external {props.brandName || props.name || 'Module Federation'}  runtime，并使用消费者提供的 runtime 。（请确保你的消费者有设置 `provideExternalRuntime: true`，否则无法正常运行！）
...
设置 `true` 后会在消费者处注入 {props.brandName || props.name || 'Module Federation'}  runtime。
