...
### 多级链路（如 A → B → C）下打开插件看不到某些 remote，怎么办？

在多级消费链路里，B 消费 C 往往是“动态注册 / 动态加载”的（例如按路由懒加载、按条件触发注册）。这会导致你**首次打开 Devtools 面板时**，插件采集模块信息的时机早于 C 实际完成注册/加载的时机，因此在 Proxy 的下拉列表或 Module Info 里暂时看不到对应的 remote。

解决方法：先在页面里触发一次该 remote 的实际加载（例如进入对应页面/执行相关操作），然后在插件面板内点击「刷新」按钮重新拉取模块信息，再进行代理配置。
