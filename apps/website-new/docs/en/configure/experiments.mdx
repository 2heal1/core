...
After setting `true`, the external {props.brandName || props.name || 'Module Federation'}  runtime will be used and the runtime provided by the consumer will be used. (Please make sure your consumer has `provideExternalRuntime: true` set, otherwise it will not run properly!)
...
Setting `true` will inject the {props.brandName || props.name || 'Module Federation'}  runtime at the consumer.
